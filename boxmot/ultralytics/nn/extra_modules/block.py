...
__all__ = ['Shareable_Multi_Semantic_Spatial_Attention', 'Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'ChannelAttention_HSFPN', 'ELA_HSFPN', 'CA_HSFPN', 'Multiply', 'High_Frequency_Perception_Module', 'DySample', 'CARAFE', 'HWD',
           ...
           'C2f_GroupMamba', 'C2f_GroupMambaBlock', 'C2f_MambaVision', 'FCM', 'FCM_1', 'FCM_2', 'FCM_3', 'Pzconv', 'MHE', 'DADM', 'CSFH_Block'
           ]
...
class DctSpatialInteraction(nn.Module):
    ...
    def forward(self, x):
        ...
        if DCT is None:
            raise ImportError('torch_dct is required for DctSpatialInteraction when isdct=True')
        ...
        dct = idct * weight  # 用权重过滤低频特征（保留高频特征）
        mask = DCT.idct_2d(dct, norm='ortho') # 生成空间掩码
        return x * mask
...
class DctChannelInteraction(nn.Module):
    ...
    def forward(self, x):
        ...
        if DCT is None:
            raise ImportError('torch_dct is required for DctChannelInteraction when isdct=True')
        idct = DCT.dct_2d(x, norm='ortho')
        weight = self._compute_weight(h, w, self.ratio).to(x.device)
        weight = weight.view(1, h, w).expand_as(idct)
        dct = idct * weight
        mask = DCT.idct_2d(dct, norm='ortho')
        amaxp = F.adaptive_max_pool2d(mask, output_size=(self.h, self.w))
        aavgp = F.adaptive_avg_pool2d(mask, output_size=(self.h, self.w))
...
class Shareable_Multi_Semantic_Spatial_Attention(nn.Module):
    def __init__(
            self,
            dim: int, # 输入特征的维度
            group_kernel_sizes: List[int] = [3, 5, 7, 9], # 分组卷积核的大小
